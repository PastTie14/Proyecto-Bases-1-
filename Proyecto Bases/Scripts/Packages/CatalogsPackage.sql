CREATE OR REPLACE PACKAGE adminCatalogs IS
    ...
    PROCEDURE updateSize(p_id_size IN NUMBER, p_name VARCHAR2);
    PROCEDURE updateSizeXCribHouse(p_id_size IN NUMBER, p_old_crib_house IN NUMBER, p_new_crib_house IN NUMBER);

    -- ==================== DELETE ====================

    PROCEDURE deleteCurrency(p_id_currency IN NUMBER);
    PROCEDURE deleteProvince(p_id_province IN NUMBER);
    PROCEDURE deleteCanton(p_id_canton IN NUMBER);
    PROCEDURE deleteDistrict(p_id_district IN NUMBER);
    PROCEDURE deletePetType(p_id_pet_type IN NUMBER);
    PROCEDURE deleteRace(p_id_race IN NUMBER);
    PROCEDURE deleteStatus(p_id_status IN NUMBER);
    PROCEDURE deleteColor(p_id_color IN NUMBER);
    PROCEDURE deleteValueType(p_id_value_type IN NUMBER);
    PROCEDURE deleteSize(p_id_size IN NUMBER);
    PROCEDURE deleteSizeXCribHouse(p_id_size IN NUMBER, p_id_crib_house IN NUMBER);
