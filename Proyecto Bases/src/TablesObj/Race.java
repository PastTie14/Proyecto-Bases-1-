...
public class Race extends DBItem {
    ...
    private void loadData() {
        ...
        try {
            ...
            LOG.log(Level.SEVERE, "Error loading Race id=" + id, ex);
        }
    }
    ...
    public int    getIdPetType() { loadData(); return getInt(2); }
    ...
    public static ResultSet getAll() {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("BEGIN ? := adminCatalogs.getRace(); END;");
            stmt.registerOutParameter(1, OracleTypes.CURSOR);
            stmt.execute();
            return (ResultSet) stmt.getObject(1);
    ...
    public static void insert(int idRace, String name, int idPetType) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL adminCatalogs.insertRace(?, ?, ?) }");
            stmt.setInt(1, idRace);
            stmt.setString(2, name);
            stmt.setInt(3, idPetType);
            stmt.execute();
        } catch (SQLException ex) { LOG.log(Level.SEVERE, null, ex); }
    }
    ...
    public void updateItem(int idRace, String name, int idPetType) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL adminCatalogs.updateRace(?, ?, ?) }");
            stmt.setInt(1, idRace);
            stmt.setString(2, name);
            stmt.setInt(3, idPetType);
            stmt.execute();
    ...
    public void deleteItem(int selectedId) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL adminCatalogs.deleteRace(?) }");
