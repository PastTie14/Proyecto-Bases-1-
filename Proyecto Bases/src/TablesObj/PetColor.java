...
public class PetColor extends DBItem {
    ...
    private void loadData() {
        ...
        try {
            ...
            LOG.log(Level.SEVERE, "Error loading Color id=" + id, ex);
        }
    }
    ...
    public static ResultSet getAll() {
        try {
            ...
            CallableStatement stmt = con.prepareCall("BEGIN ? := adminCatalogs.getColor(); END;");
    ...
    public static void insert(int idColor, String name) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL adminCatalogs.insertColor(?, ?) }");
    ...
    public void updateItem(int idColor, String name) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL adminCatalogs.updateColor(?, ?) }");
            stmt.setInt(1, idColor);
            stmt.setString(2, name);
            stmt.execute();
            data = null;
    ...
    public void deleteItem(int selectedId) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL adminCatalogs.deleteColor(?) }");
