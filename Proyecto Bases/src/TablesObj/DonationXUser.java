...
public class DonationXUser extends DBItem {
    ...
    public static ResultSet getAll() {
        try {
            ...
            CallableStatement stmt = con.prepareCall("BEGIN ? := adminFinancial.getDonationXUser(); END;");
    ...
    public static void insert(int idUser, int idDonation) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL adminFinancial.insertDonationXUser(?, ?) }");
            stmt.setInt(1, idUser);
            stmt.setInt(2, idDonation);
            stmt.execute();
        } catch (SQLException ex) { LOG.log(Level.SEVERE, null, ex); }
    }
